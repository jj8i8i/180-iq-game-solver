...
function displayResults(result) {
    // --- NEW: Filter out visually duplicate solutions ---
    const seenStrings = new Set();
    const uniqueSolutions = result.solutions.filter(sol => {
        if (seenStrings.has(sol.str)) {
            return false;
        }
        seenStrings.add(sol.str);
        return true;
    });
...
function generateDerivationList(item) {
    ...
    for (const sub of subExpressions) {
        let replaced = false;
        // A more robust way to replace to avoid issues with non-unique substrings
        const regex = new RegExp(sub.str.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'));
        const newEquation = currentEquation.replace(regex, () => {
            if (replaced) return sub.str;
            replaced = true;
            return sub.derivation.resultStr;
        });
